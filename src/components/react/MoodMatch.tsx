  ...
  return (
    <section className="relative w-full max-w-6xl mx-auto my-6 sm:my-8 px-3 sm:px-4">
      ...
      <div className="relative rounded-3xl p-4 sm:p-6 overflow-hidden border border-purple-500/20 bg-gradient-to-b from-[#0f0c1b]/95 to-[#07060c]/98 backdrop-blur-2xl shadow-2xl shadow-purple-950/30 space-y-4 sm:space-y-5">
        ...
        {/* Loading Skeleton */}
        {loading && !result && (
          <div className="pt-3 border-t border-purple-500/15 animate-pulse space-y-4">
            <div className="flex items-center justify-between">
              <div className="h-5 w-56 bg-purple-950/60 rounded-xl" />
              <div className="h-4 w-28 bg-purple-950/40 rounded-full" />
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3 sm:gap-4">
              {[...Array(6)].map((_, i) => (
                <div key={i} className="rounded-2xl bg-black/40 border border-purple-500/15 overflow-hidden flex flex-col p-2 space-y-2">
                  <div className="aspect-[2/3] w-full rounded-xl bg-purple-950/40" />
                  <div className="h-3.5 w-3/4 bg-purple-950/60 rounded" />
                  <div className="h-2.5 w-1/2 bg-purple-950/30 rounded" />
                  <div className="h-8 w-full bg-purple-950/20 rounded-lg" />
                  <div className="h-7 w-full bg-purple-950/50 rounded-lg" />
                </div>
              ))}
            </div>
          </div>
        ...
        {/* Results Showcase */}
        {result && (
          <div className="space-y-4 pt-3 border-t border-purple-500/15 animate-in fade-in">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              ...
              <span className="text-[10px] font-mono font-bold text-purple-300 self-start sm:self-auto uppercase tracking-wider px-3 py-1 rounded-full bg-purple-950/60 border border-purple-500/30 shadow-inner flex items-center gap-1.5">
                <span className="w-1.5 h-1.5 rounded-full bg-purple-400 animate-pulse" />
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3 sm:gap-4">
              {result.picks.map((pick) => {
                const isSaved = savedIds.includes(pick.id);
                const isSeries = pick.mediaType === 'tv' || pick.mediaType === 'series';
                return (
                  <div
                    key={pick.id}
                    className="group relative rounded-2xl bg-gradient-to-b from-[#130f24]/90 via-[#0a0815]/95 to-[#040308]/98 border border-purple-500/25 hover:border-purple-400/60 overflow-hidden flex flex-col transition-all duration-300 shadow-[0_8px_30px_rgba(0,0,0,0.7)] hover:shadow-[0_16px_50px_rgba(147,51,234,0.35)] hover:-translate-y-1.5"
                  >
                    {/* Poster with Dual Gradient Scrim & Hover Play */}
                    <a
                      href={pick.href}
                      className="relative aspect-[2/3] w-full overflow-hidden bg-black/80 block focus:outline-none"
                    >
                      {pick.posterUrl ? (
                        <img
                          src={pick.posterUrl}
                      ...
                      {/* Bottom Scrim */}
                      <div className="absolute inset-x-0 bottom-0 h-20 bg-gradient-to-t from-black via-black/60 to-transparent pointer-events-none" />

                      {/* Top Badges */}
                      <div className="absolute top-2 left-2 z-10 flex items-center gap-1">
                        <div className="px-2 py-0.5 rounded-full bg-black/85 backdrop-blur-md text-[9px] font-black tracking-wide text-emerald-300 border border-emerald-500/40 shadow-lg flex items-center gap-1">
                          <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
                          <span>{pick.matchPercent || '98% Match'}</span>
                        </div>
                      </div>

                      <div className="absolute top-2 right-2 z-10">
                        <div className="px-2 py-0.5 rounded-full bg-black/85 backdrop-blur-md text-[10px] font-mono font-bold text-amber-300 flex items-center gap-1 border border-amber-500/30 shadow-lg">
                          <span className="text-amber-400">★</span>
                          <span>{pick.rating.toFixed(1)}</span>
                        </div>
                      </div>

                      {/* Bottom Poster Badges: Media Type & HD */}
                      <div className="absolute bottom-2 left-2 right-2 z-10 flex items-center justify-between pointer-events-none">
                        <span className="px-1.5 py-0.5 rounded-md bg-purple-950/90 backdrop-blur-md text-[9px] font-black uppercase tracking-wider text-purple-200 border border-purple-400/40 shadow-md">
                          {isSeries ? 'Series' : 'Movie'}
                        </span>
                        <span className="px-1.5 py-0.5 rounded-md bg-white/10 backdrop-blur-md text-[9px] font-black uppercase tracking-wider text-white/80 border border-white/15">
                          HD
                        </span>
                      </div>
                      ...
                      <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none z-20">
                        <div className="w-11 h-11 sm:w-12 sm:h-12 rounded-full bg-purple-600/90 text-white flex items-center justify-center shadow-[0_0_30px_rgba(168,85,247,0.8)] transform scale-90 group-hover:scale-100 transition-transform duration-300">
                    ...
                    <div className="p-2.5 sm:p-3 flex-1 flex flex-col justify-between space-y-2.5">
                      <div className="space-y-1">
                        <a
                          href={pick.href}
                          className="text-xs sm:text-sm font-black text-white hover:text-purple-300 line-clamp-1 transition-colors block"
                          title={pick.title}
                        >
                          {pick.title}
                        </a>
                        <div className="flex items-center gap-1.5 text-[10px] text-white/45 font-mono">
                          <span>{pick.year}</span>
                          <span>•</span>
                          <span className="text-purple-300/80 font-semibold">{isSeries ? 'TV Series' : 'Cinema Film'}</span>
                        </div>

                        {/* AI "Why It Fits" Insight Pill */}
                        <div className="mt-2 p-2 rounded-xl bg-purple-950/40 border border-purple-500/20 group-hover:border-purple-400/40 transition-colors">
                          <div className="flex items-center gap-1 text-[9px] font-black uppercase tracking-wider text-purple-300 mb-1">
                            <svg className="w-2.5 h-2.5 text-purple-400 flex-none" fill="currentColor" viewBox="0 0 24 24">
                              <path d="M12 2l2.4 7.2L21 12l-6.6 2.8L12 22l-2.4-7.2L3 12l6.6-2.8L12 2z" />
                            </svg>
                            <span>AI Vibe Match</span>
                          </div>
                          <p className="text-[10.5px] text-purple-100/85 leading-snug line-clamp-2">
                      ...
                      <div className="flex items-center gap-1.5 pt-1">
                        <a
                          href={pick.href}
                          className="flex-1 py-2 rounded-xl bg-gradient-to-r from-purple-600 to-purple-700 hover:from-purple-500 hover:to-purple-600 text-white text-[11px] font-bold text-center transition-all flex items-center justify-center gap-1.5 shadow-md shadow-purple-950/50 active:scale-95"
                        >
                        ...
                        <button
                          ...
                          className={`w-8.5 h-8.5 px-2 rounded-xl text-xs font-bold border transition-all flex items-center justify-center active:scale-90 ${
                            isSaved
                              ? 'bg-purple-500/30 text-purple-300 border-purple-400 shadow-sm'
